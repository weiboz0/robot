...
| `GET /pose_trail` | `{"trail":[[x,y],…],"pose":{…}}` — driven path (bounded, ≥5 cm spacing, origin-seeded) + the same dict `/pose` serves; the 🗺 Map tab draws this, and the chatbot's `rover_come_back` backtracks along it. Chatbot driving ("go to the suitcase", "come back") needs `ROVER_GO_ENABLE=1` — a separate consent from `ROVER_FIND_ENABLE` (camera sweep + in-place rotation only); every forward pulse AND turn is behind a fresh vision floor-check. Go-to looks all around first (gated rotations, ~a full circle as time allows) and detours around blocked floor (bounded; stops honestly when boxed in). Floor gates are motion-typed (plan 039): forward pulses judge the center DRIVE STRIP (side clutter noted but not blocking); turns keep full-frame ~30 cm strictness; a person or animal ANYWHERE in view always blocks; every verdict is logged |
